...
def extrapolate_wind_speed(
    ...
    if baseline_height <= 0 or target_height <= 0:
        raise ValueError("Heights must be strictly positive values.")
...
# ==============================================================================
TURBINE_SPEED_POINTS = np.array([0.0, 3.0, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 11.0, 25.0, 25.1, 40.0])
TURBINE_POWER_OUTPUTS = np.array([0.0, 0.0, 1.5, 3.0, 5.2, 7.8, 10.5, 12.8, 14.5, 15.0, 15.0, 0.0, 0.0])
...
def get_turbine_power_generation(wind_speeds_hub: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    """Maps continuous wind speed arrays directly to individual turbine output (MW)."""
    return np.clip(_calculate_turbine_power(wind_speeds_hub), 0.0, 15.0)

# ==============================================================================
# 3. METOCEAN LOGISTICS & WAVE ACCESSIBILITY
# ==============================================================================
def model_significant_wave_height(wind_speeds_ms: np.ndarray, seed: int = 42) -> np.ndarray:
    """Empirical metocean relationship mapping wind shear force to significant wave heights."""
    if len(wind_speeds_ms) == 0:
...
def evaluate_marine_accessibility(wave_heights_array: np.ndarray) -> Tuple[float, float]:
...
# ==============================================================================
def fit_extreme_storm_h50(
    annual_max_waves: np.ndarray, 
    return_period_years: float = 50.0
) -> Tuple[float, float, float]:
    """Fits a Gumbel distribution to annual maxima to extrapolate design wave survival limits."""
    if len(annual_max_waves) < 2:
        raise ValueError("Insufficient data points to perform Gumbel extreme value fitting.")
    ...
    return float(design_threshold_h50), float(fitted_loc), float(fitted_scale)

# ==============================================================================
# 5. GEOTECHNICAL FOUNDATION SCOUR MITIGATION
# ==============================================================================
def design_monopile_scour_protection(
    monopile_diameter: float = 10.0, 
    rock_density: float = 2650.0, 
    rock_d50: float = 0.45
) -> Tuple[float, float, float]:
    ...
    # 0.35 --- typical rock aggregate porosity
    solid_mass_tonnes = (bulk_volume * (1.0 - 0.35) * rock_density) / 1000.0
    return equilibrium_scour_depth, bulk_volume, solid_mass_tonnes

# ==============================================================================
# 6. CENTRAL DIFFERENCE CABLE SLOPE GEOMETRY
# ==============================================================================
def analyze_cable_corridor_slopes(Z_corridor_matrix: np.ndarray, spacing_m: float = 1.0) -> np.ndarray:
    """Computes localized terrain slope angles using spatial central differences."""
    if spacing_m <= 0:
        raise ValueError("Spatial grid spacing must be greater than zero.")
...
# ==============================================================================
def calculate_catenary_profile(T_h: float, weight_pm: float, depth: float = 120.0) -> Tuple[np.ndarray, np.ndarray]:
    """Solves the hyperbolic cosine profile equations describing catenary line geometry."""
    if weight_pm <= 0:
        raise ValueError("Submerged weight per meter must be positive.")
        
    a = T_h / weight_pm
    
  
    val_to_cosh = (depth + a) / a
    max_x = a * np.arccosh(val_to_cosh if val_to_cosh >= 1.0 else 1.0)
...
# ==============================================================================
def run_lifecycle_financial_model(
    net_annual_gwh: float, 
    capacity_mw: float = 15.0, 
    years: int = 25
) -> Tuple[float, float]:
    """Evaluates CapEx, OpEx, asset payback windows, and overall project lifetime LCOE."""
    total_capex = capacity_mw * 3_500_000.0
    annual_opex = capacity_mw * 80_000.0
    lifetime_mwh = (net_annual_gwh * 1000.0) * years
    
    if lifetime_mwh == 0:
        return 0.0, 0.0
        
    lcoe_per_mwh = (total_capex + (annual_opex * years)) / lifetime_mwh
    annual_revenue = (net_annual_gwh * 1000.0) * 75.0
    
    net_annual_cash_flow = annual_revenue - annual_opex
    payback_years = total_capex / net_annual_cash_flow if net_annual_cash_flow > 0 else float('inf')
    
    return float(lcoe_per_mwh), float(payback_years)
